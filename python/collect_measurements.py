...
def main():
    with open("/home/andrei/Dev/RouteFindingSimulator/measurements/test_commtime2.csv",'w') as outfile:
    ...
    node_counts = np.round(10 * (2 ** np.linspace(0, 5, 7))) #[10,40,160,640]
    ...
    for seeds, node_count, edge_percent, pe_grid_size, algorithm in configs:
        result = subprocess.run(get_command_to_run("/home/andrei/Dev/RouteFindingSimulator/measurements/test_commtime2.csv", seeds,node_count,edge_percent,pe_grid_size,algorithm), capture_output=True)
        progress+=1
