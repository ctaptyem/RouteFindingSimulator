...
def main():
    output_file_name = "new_model_commtime"
    with open(f"/home/andrei/Dev/RouteFindingSimulator/measurements/{output_file_name}.csv",'w') as outfile:
        outfile.write("algorithm,pe_grid_size,node_count,edge_percentage,undirected,weight_mean,weight_std,edge_seed,weight_seed,runtime,commtime,commcount,commvolume,total_read,total_write\n")
    ...
    for seeds, node_count, edge_percent, pe_grid_size, algorithm in configs:
        ...
        result = subprocess.run(get_command_to_run(f"/home/andrei/Dev/RouteFindingSimulator/measurements/{output_file_name}.csv", seeds,node_count,edge_percent,pe_grid_size,algorithm), capture_output=True)
        progress+=1
    print()
